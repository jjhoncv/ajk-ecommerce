...
export class SearchRepository {
  public async searchProductVariants(
    ...
    try {
      // Construir SELECT dinamicamente segun si hay filtros de promocion
    ...
    `
      ...
      `)
        ...
        // Tambien filtrar por promociones activas
        whereConditions.push(`
        prom.is_active = 1
      ...
      throw new Error(
        `Error en busqueda de productos: ${error instanceof Error ? error.message : 'Error desconocido'}`
      )
    }
  }
  ...
  public async getSearchSuggestions(
    ...
    try {
      ...
      // Solo tomar terminos de 2+ caracteres, sin filtros complicados
      ...
      // Siempre usar busqueda simple con el primer termino valido
      // Esto es mas estable y predecible
      ...
      >({
        query: `
          SELECT DISTINCT suggestion, priority FROM (
            -- Productos que contienen el termino
            ...
            -- Productos + atributos especificos
