...
// DTOs types (fallback since @/dto doesn't exist)
    ...
    variant.variantAttributeOptions.forEach((variantAttrOption) => {
      if (variantAttrOption?.productAttributeOption?.productAttributeOptionImages) {
        const attrOptionImages =
          variantAttrOption.productAttributeOption.productAttributeOptionImages
            .filter((img): img is ProductAttributeOptionImages => !!img)
            .map(
              (img): ItemImage => ({
