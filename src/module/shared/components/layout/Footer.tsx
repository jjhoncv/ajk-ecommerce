...
import paymentMethodModel from '@/module/payments/core/PaymentMethod.model'
...
const Footer = async (): Promise<JSX.Element> => {
  const [informations, pages, mainCategories, storeConfig, socialLinks, paymentMethods] =
    await Promise.all([
      InformationService.getInformation(),
      PageService.getPages(),
      CategoryService.getMainCategories(),
      StoreConfigService.getStoreConfig(),
      StoreConfigService.getSocialLinks(),
      paymentMethodModel.getPaymentMethods()
    ])

  const hasStoreInfo = storeConfig && (storeConfig.storeName || storeConfig.logoUrl)
  const hasInformations = informations.length > 0
  const hasSocialLinks = socialLinks.length > 0
  const hasCategories = mainCategories.length > 0
  const hasPages = pages.length > 0
  const hasPaymentMethods = paymentMethods && paymentMethods.length > 0

  const hasAnyContent = hasStoreInfo || hasCategories || hasPages || hasPaymentMethods

  if (!hasAnyContent) {
    return <></>
  }

  const activePaymentMethods = paymentMethods?.filter((m) => m.isActive === 1) || []
          ...
          {hasStoreInfo && (
            <div>
              <div className="mb-4 mt-4">
                {storeConfig.logoUrl ? (
                  // Si hay logo, mostrar solo la imagen con dimensiones configuradas
                  <Link href="/">
                    <Image
                      src={storeConfig.logoUrl}
                      alt={storeConfig.storeName || 'Logo'}
                      width={storeConfig.logoWidth || 150}
                      height={storeConfig.logoHeight || 50}
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {storeConfig.storeDescription && (
                      <p className="text-sm text-gray-600">{storeConfig.storeDescription}</p>
                    )}
                  </div>
                )}
              </div>
                ...
                <div className="flex gap-4">
                  {socialLinks.map((link) => (
                    <a
                      ...
                      dangerouslySetInnerHTML={{ __html: SOCIAL_ICONS[link.type] || '' }}
                    />
                  ))}
                </div>
              )}
          ...
          <div className="mx-auto max-w-screen-4xl px-12 py-4">
            <div className="flex flex-col items-center justify-between md:flex-row">
              <p className="mb-4 text-gray-600 md:mb-0">
                © {new Date().getFullYear()} {storeConfig.storeName || 'Mi Tienda'}. Todos los
                derechos reservados
              </p>
              {storeConfig.phone && (
                <div className="flex items-center gap-4">
                  <p className="text-gray-600">
                    Línea de atención: {storeConfig.phone}
                  </p>
                </div>
              )}
            </div>
