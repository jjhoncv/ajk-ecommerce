...
export default function CouponsListAdmin({ initialCoupons, stats }: CouponsListAdminProps) {
  ...
  const filteredCoupons = useMemo(() => {
    ...
    return filtered
  }, [initialCoupons, searchTerm, statusFilter])
  ...
  const localStats = useMemo(() => {
    const active = initialCoupons.filter((c) => getCouponStatus(c) === 'active').length
    const totalUsages = initialCoupons.reduce((sum, c) => sum + (Number(c.totalUsages) || 0), 0)
    const totalDiscount = initialCoupons.reduce((sum, c) => sum + (Number(c.totalDiscountAmount) || 0), 0)
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <div className="rounded-lg border bg-blue-50 p-3">
          <div className="text-xl font-bold text-blue-700">{initialCoupons.length}</div>
