...
export default function StockValidationModal({
  ...
  const reducedStockItems = errors.filter(error => error.availableStock > 0 && error.availableStock < error.requestedQuantity)
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      className="p-4 pt-4 max-w-lg" // Un poco más ancho para el contenido
    >
      ...
      <ModalContent className="p-0">
        <div className="space-y-4">
          ...
          <div className="space-y-4 max-h-80 overflow-y-auto"> {/* Scroll si hay muchos productos */}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {outOfStockItems.map((error) => (
                    <div key={error.variantId} className="bg-red-50 border border-red-200 p-3 rounded-md">
                      <p className="text-sm text-red-800 font-medium mb-1">
                        {error.productName}
                      </p>
                      <p className="text-xs text-red-600 mb-1">
                        Solicitado: <span className="font-medium">{error.requestedQuantity}</span> •
                        Disponible: <span className="font-medium">{error.availableStock}</span>
                      </p>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {reducedStockItems.map((error) => (
                    <div key={error.variantId} className="bg-orange-50 border border-orange-200 p-3 rounded-md">
                      ...
                      <p className="text-xs text-orange-600">
                        ⚠️ Por favor, reduce la cantidad a máximo {error.availableStock} unidad{error.availableStock !== 1 ? 'es' : ''}
                      </p>
                    </div>
                  ))}
                </div>
          ...
          <div className="flex gap-3 justify-end pt-4 border-t">
            <button
              onClick={() => onCancel?.()}
              className="px-4 py-2 text-gray-700 border font-semibold text-sm border-gray-300 hover:bg-gray-50 transition-colors rounded-md"
            >
              Cancelar
            </button>
            ...
            <button
              ...
              className="px-4 py-2 bg-indigo-600 text-white font-semibold text-sm hover:bg-indigo-700 transition-colors rounded-md"
            >
              Entendido
            </button>
          </div>
