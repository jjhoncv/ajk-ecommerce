...
export default function CartPageContent() {
  const { items, totalItems, isInitialized, openDeleteConfirmation, removeItem, updateQuantity } = useCartContext();

  // 🆕 Estado para almacenar información de stock actualizada desde validación
  const [stockInfo, setStockInfo] = useState<Array<{ id: number; availableStock: number }>>([]);
    ...
    if (isInitialized) {
      try {
        ...
        if (savedStockInfo) {
          ...
          console.log('📦 Restored stock validation info from localStorage:', parsedStockInfo);
          setStockInfo(parsedStockInfo);
        } else {
          console.log('📭 No stock validation info found in localStorage');
        }
      } catch (error) {
        console.error('❌ Error loading stock validation info from localStorage:', error);
      }
    }
  }, [isInitialized]);
  ...
  const handleStockInfoUpdate = (newStockInfo: Array<{ id: number; availableStock: number }>) => {
    ...
    const updatedCart = currentCart.map((cartItem: any) => {
      ...
      if (stockUpdate) {
        ...
        console.log(`   Stock: ${cartItem.stock} → ${stockUpdate.availableStock}`);
  ...
  const handleCartUpdate = (adjustedItems: Array<{ id: number; quantity: number }>) => {
    ...
    adjustedItems.forEach(adjustment => {
      if (adjustment.quantity === 0) {
        ...
        console.log(`🔄 Updating item ${adjustment.id} quantity to ${adjustment.quantity}`);
